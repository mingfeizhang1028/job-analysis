...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr  3 18:50:53 2026
...
CFPS_RAW = Path(r"O:\日常文件\毕业论文\01.数据\CFPS\Stata14")
OUT = Path(r"O:\日常文件\毕业论文\01.数据\CFPS\待处理")
MAC = Path(r"O:\日常文件\毕业论文\01.数据\1. 宏观变量")

# 输出目录
EXPORT_DIR = OUT / "metadata_exports"
...
# 是否读取数据内容来计算非缺失、唯一值、最小值、最大值
# 若文件较大，可设为 False，仅做 metadata 扫描
COMPUTE_COLUMN_STATS = True

# 唯一值数若太多，计算会慢；可限制只对前若干行估计，或全样本
UNIQUE_COUNT_SAMPLE_LIMIT = None  # None 表示全样本；例如 50000 表示最多抽样 50000 行
...
def infer_year_from_name(file_name: str):
    """
    从文件名中识别年份，如 cfps2022person_202410.dta -> 2022
    """
    m = re.search(r"(20\d{2}|19\d{2})", file_name)
    return int(m.group(1)) if m else None


def infer_module_type(file_name: str, parent_dir: str = ""):
    """
    粗略识别文件类型
    """
    ...
    if "community" in s:
        return "community"
    if "macro" in s or "省级" in s or "宽带" in s or "高铁" in s:
        return "macro"
    return "unknown"
...
def build_metadata_tables(file_list):
    ...
    for dir_tag, fp in file_list:
        ...
        try:
            ...
            module_type = infer_module_type(file_name, fp.parent.name)
            ...
            file_inventory_rows.append({
                ...
                "module_type": infer_module_type(fp.name, fp.parent.name),
                "n_rows": None,
                "n_cols": None,
    ...
    if not variable_dictionary.empty:
        ...
        tmp["year_str"] = tmp["year"].astype("Int64").astype(str)
        var_year_summary = (
            ...
            .agg(
                ...
                first_year=("year", "min"),
                last_year=("year", "max"),
                module_types=("module_type", lambda x: ",".join(sorted(set(map(str, x.dropna().tolist()))))),
...
def build_processing_map_template():
    """
    根据你当前 Stata 代码，预置一版处理映射骨架。
    这不是 100% 自动还原，而是“研究可维护模板”。
    """
    ...
    rows = [
        ...
        {
            "final_var": "job_sat_overall",
            "raw_var": "qg406",
            "source_file_pattern": "cfps2020person, cfps2022person, cfps2014/2016 via rename",
            "source_years": "2014,2016,2020,2022",
            "transform_type": "rename_if_exists",
            "transform_detail": "qg406 -> job_sat_overall",
            "stata_program": "_make_demographic",
            "intermediate_var": "",
            "used_in_analysis": 0,
            "analysis_block": "备选机制变量",
            "comparability_risk": "中",
            "notes": "可作为工作质量机制检验候选。"
        },
        ...
        # --------------------------
        {
            "final_var": "feduy1 / meduy1 / foccup / moccup / fpoli_status / mpoli_status",
            "raw_var": "qv102/qv103code/qv104/qv202/qv203code/qv204 等",
            "source_file_pattern": "cfps2012adult, cfps2018person, cfps2020person, cfps2022person",
            "source_years": "2012,2018,2020,2022",
            "transform_type": "rename_harmonize",
            "transform_detail": "父母教育/职业/政治变量统一命名",
            "stata_program": "year-specific rename blocks",
            "intermediate_var": "",
            "used_in_analysis": 1,
            "analysis_block": "先赋条件；IOp",
            "comparability_risk": "中",
            "notes": "2010/2012/2014/2016/2018 来源并不完全一致，需谨慎拼接。"
        },
        {
            "final_var": "fedu / medu / feduc / meduc",
            "raw_var": "feduc/feduy2, meduc/meduy2 等",
            "source_file_pattern": "processed panel",
            "source_years": "2010-2022",
            "transform_type": "rowmax_then_years_mapping",
            "transform_detail": "先按 pid 聚合父母教育最大值，再映射为学年",
            "stata_program": "main do-file",
            "intermediate_var": "fedu_rowmax, medu_rowmax",
            "used_in_analysis": 1,
            "analysis_block": "先赋条件；IOp",
            "comparability_risk": "中高",
            "notes": "不同年份父母教育变量来源多样，建议单列附录解释。"
        },
        {
            "final_var": "foccu / moccu / fpoli_sta / mpoli_sta",
            "raw_var": "foccup / moccup / fpoli_status / mpoli_status",
            "source_file_pattern": "processed panel",
            "source_years": "2010-2022",
            "transform_type": "within_pid_max",
            "transform_detail": "bys pid: egen max(...)",
            "stata_program": "main do-file",
            "intermediate_var": "",
            "used_in_analysis": 1,
            "analysis_block": "先赋条件；IOp",
        ...
        {
            "final_var": "trust_general",
            "raw_var": "qn1001 / pn1001",
            "source_file_pattern": "cfps2016-2022 adult/person",
            "source_years": "2016,2020,2022",
            "transform_type": "fill_merge",
            "transform_detail": "若 trust_general 不存在则生成，优先 qn1001，再用 pn1001 回填",
            "stata_program": "_make_demographic",
            "intermediate_var": "",
            "used_in_analysis": 0,
            "analysis_block": "备选机制变量",
        ...
        {
            "final_var": "adult_count / child_count / old_count / child_p / old_p",
            "raw_var": "age + fid + year",
            "source_file_pattern": "processed panel",
            "source_years": "2010-2022",
            "transform_type": "family_aggregate",
            "transform_detail": "按 fid-year 统计家庭年龄结构",
            "stata_program": "main do-file",
            "intermediate_var": "adult, child, old",
            "used_in_analysis": 1,
            "analysis_block": "家庭控制变量",
            "comparability_risk": "低",
            "notes": ""
        },
        ...
        # --------------------------
        {
            "final_var": "internet, hukou, urban, unhealth, gender, job, jobclass, marriage, medsure_dum, nonfarm 等",
            "raw_var": "原变量自身",
            "source_file_pattern": "processed panel",
            "source_years": "2010-2022",
            "transform_type": "panel_binary_imputation",
            "transform_detail": "按 pid 在相邻两期都非缺失时用前值补中间缺失",
            "stata_program": "impute_binary_pid",
            "intermediate_var": "v_tmp",
            "used_in_analysis": 1,
            "analysis_block": "缺失处理",
            "comparability_risk": "中",
            "notes": "需单独生成插补比例报告。"
        ...
        {
            "final_var": "Wage, Wage_m, incomea, incomeb, educ, health, work_importance 等",
            "raw_var": "原变量自身",
            "source_file_pattern": "processed panel",
            "source_years": "2010-2022",
            "transform_type": "panel_linear_imputation",
            "transform_detail": "按 pid 用前后相邻期均值补中间缺失",
            "stata_program": "impute_linear_pid",
            "intermediate_var": "v_tmp",
            "used_in_analysis": 1,
            "analysis_block": "缺失处理",
            "comparability_risk": "中高",
            "notes": "连续变量插补可能影响标准误和分布，建议稳健性中报告不插补结果。"
        },
    ]
...
def main():
    ...
    metadata_xlsx = EXPORT_DIR / "cfps_metadata.xlsx"
    ...
    processing_xlsx = EXPORT_DIR / "cfps_processing_map.xlsx"
    export_processing_excel(processing_map, final_var_list, risk_template, processing_xlsx)

    print(f"[完成] 处理映射模板已导出：{processing_xlsx}")

    # 同时导出 CSV，方便版本控制
    file_inventory.to_csv(EXPORT_DIR / "file_inventory.csv", index=False, encoding="utf-8-sig")
    variable_dictionary.to_csv(EXPORT_DIR / "variable_dictionary.csv", index=False, encoding="utf-8-sig")
    value_labels.to_csv(EXPORT_DIR / "value_labels.csv", index=False, encoding="utf-8-sig")
    var_year_summary.to_csv(EXPORT_DIR / "var_year_summary.csv", index=False, encoding="utf-8-sig")
    processing_map.to_csv(EXPORT_DIR / "processing_map_template.csv", index=False, encoding="utf-8-sig")
    final_var_list.to_csv(EXPORT_DIR / "final_variable_list.csv", index=False, encoding="utf-8-sig")
    risk_template.to_csv(EXPORT_DIR / "comparability_risk_template.csv", index=False, encoding="utf-8-sig")
