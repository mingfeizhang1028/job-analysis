...
# -*- coding: utf-8 -*-
"""
CFPS 变量协调半自动系统（按相似文件来源高效比对版）
版本：Python 3.10+
作者：ChatGPT（按用户研究框架定制）
用途：
1．读取已有元数据 Excel：cfps_metadata.xlsx
2．构建 variable_cards enriched
3．仅针对 CFPS_RAW 中“相同 dataset_class + 相同 survey_role”的跨年变量生成候选变量对
4．为候选对构造 LLM prompt
...
依赖：
pip install pandas numpy openpyxl xlsxwriter rapidfuzz requests tqdm
...
本版核心限制：
1．仅比较 dir_tag == CFPS_RAW 的变量；
2．仅比较 dataset_class 相同的变量；
3．仅比较 survey_role 相同的变量；
4．仅比较不同年份之间的变量；
5．OUT 与 MAC 不参与比对。
"""
...
# 候选对分块写盘参数
# =========================
WRITE_CANDIDATES_STREAMING = True
CANDIDATE_FLUSH_SIZE = 50000          # 累积到多少条就写盘一次
CANDIDATE_MIN_SCORE = 0.0             # 可按需提高，如 0.15 / 0.2
CANDIDATE_TOPK_PER_LEFT = 20          # 每个左侧变量最多保留前K个候选，建议 10~30
CANDIDATE_CSV_NAME = "candidate_pairs.csv"

# 若不想每次覆盖，可改为 False；但通常建议重跑时覆盖
OVERWRITE_CANDIDATE_FILE = True
...
MAX_YEAR_GAP = None   # 如设为 8，则只比较年份差 <= 8；设为 None 表示不限制

# 比对范围约束
ONLY_COMPARE_DIR_TAG = "CFPS_RAW"
REQUIRE_SAME_DATASET_CLASS = True
REQUIRE_SAME_SURVEY_ROLE = True
REQUIRE_CROSS_YEAR_ONLY = True

# 是否跳过 raw 中一些不想比较的 dataset_class
EXCLUDE_DATASET_CLASSES = {
    # 如你未来不想比较 id_link / family_link，可取消注释：
    # "cfps_id_link",
    # "cfps_family_link",
}
...
def build_variable_cards_from_metadata(
    ...
    vdict["dir_tag"] = vdict["dir_tag"].fillna("").astype(str)
    vdict["source_stage"] = vdict["source_stage"].fillna("").astype(str)
    vdict["dataset_class"] = vdict["dataset_class"].fillna("").astype(str)
    vdict["survey_role"] = vdict["survey_role"].fillna("").astype(str)
    vdict["path_group"] = vdict["path_group"].fillna("").astype(str)
    vdict["source_note"] = vdict["source_note"].fillna("").astype(str)
    ...
    cards["value_labels_text"] = cards["value_labels_text"].fillna("")
...
def hard_compare_scope_filter(a: pd.Series, b: pd.Series) -> bool:
    # 不与自己比
    if a["card_id"] == b["card_id"]:
        return False

    # 仅比较 CFPS_RAW
    if str(a.get("dir_tag", "")) != ONLY_COMPARE_DIR_TAG:
        return False
    if str(b.get("dir_tag", "")) != ONLY_COMPARE_DIR_TAG:
        return False

    # 排除指定 dataset_class
    if str(a.get("dataset_class", "")) in EXCLUDE_DATASET_CLASSES:
        return False
    if str(b.get("dataset_class", "")) in EXCLUDE_DATASET_CLASSES:
        return False

    # dataset_class 必须相同
    if REQUIRE_SAME_DATASET_CLASS:
        if str(a.get("dataset_class", "")) != str(b.get("dataset_class", "")):
            return False

    # survey_role 必须相同
    if REQUIRE_SAME_SURVEY_ROLE:
        if str(a.get("survey_role", "")) != str(b.get("survey_role", "")):
            return False

    # 仅跨年比较
    a_year = safe_int(a.get("year"))
    b_year = safe_int(b.get("year"))
    if REQUIRE_CROSS_YEAR_ONLY:
        if a_year is not None and b_year is not None and a_year == b_year:
            return False

    # 可选的年份跨度限制
    if MAX_YEAR_GAP is not None and a_year is not None and b_year is not None:
        if abs(a_year - b_year) > MAX_YEAR_GAP:
            return False

    return True


def rough_similarity_filter(a: pd.Series, b: pd.Series) -> bool:
    a_name = normalize_text(a.get("var_name", ""))
    b_name = normalize_text(b.get("var_name", ""))
    a_label = normalize_text(a.get("var_label", ""))
    b_label = normalize_text(b.get("var_label", ""))
...
def should_compare(a: pd.Series, b: pd.Series) -> bool:
    if not hard_compare_scope_filter(a, b):
        return False
    if not rough_similarity_filter(a, b):
        return False
    return True


def generate_candidate_pairs(cards: pd.DataFrame) -> pd.DataFrame:
    # 先做比对范围预过滤：仅保留 CFPS_RAW
    work = cards.copy()
    work = work[work["dir_tag"].astype(str) == ONLY_COMPARE_DIR_TAG].copy()

    if EXCLUDE_DATASET_CLASSES:
        work = work[~work["dataset_class"].astype(str).isin(EXCLUDE_DATASET_CLASSES)].copy()

    if work.empty:
        return pd.DataFrame()

    # 仅保留有意义的分组键
    group_cols = ["dataset_class", "survey_role"] if REQUIRE_SAME_SURVEY_ROLE else ["dataset_class"]

    records = []

    grouped = work.groupby(group_cols, dropna=False)

    for group_key, g in tqdm(grouped, desc="按相似文件来源分组比对"):
        ...
        if "year" in g.columns:
            g = g.sort_values(by=["year", "file_name", "var_name"], na_position="last").reset_index(drop=True)
        ...
        for i in range(n):
            ...
            for j in range(i + 1, n):
                ...
                rec = {
                    ...
                    "group_dataset_class": a.get("dataset_class"),
                    "group_survey_role": a.get("survey_role"),

                    "a_card_id": a["card_id"],
                    "a_dir_tag": a.get("dir_tag"),
...
def build_llm_prompt(pair_row: pd.Series) -> str:
    content = {
        ...
        "variable_a": {
            "dir_tag": truncate_for_prompt(pair_row.get("a_dir_tag")),
        ...
        "variable_b": {
            "dir_tag": truncate_for_prompt(pair_row.get("b_dir_tag")),
...
def build_review_template(
    ...
    base_cols = [
        "pair_id",
        "group_dataset_class", "group_survey_role",
...
def main():
    ...
    # 打印范围统计，便于核验
    print("-" * 80)
    print("第 2.5 步：比对范围核验")
    if "dir_tag" in variable_cards.columns:
        print("dir_tag 分布：")
        print(variable_cards["dir_tag"].value_counts(dropna=False).to_string())
    if {"dir_tag", "dataset_class", "survey_role"}.issubset(variable_cards.columns):
        tmp_scope = variable_cards[variable_cards["dir_tag"].astype(str) == ONLY_COMPARE_DIR_TAG].copy()
        if EXCLUDE_DATASET_CLASSES:
            tmp_scope = tmp_scope[~tmp_scope["dataset_class"].astype(str).isin(EXCLUDE_DATASET_CLASSES)].copy()
        scope_tab = (
            tmp_scope.groupby(["dataset_class", "survey_role"], dropna=False)
            .size()
            .reset_index(name="n_vars")
            .sort_values(by=["dataset_class", "survey_role"])
        )
        print("实际进入候选生成的组别统计：")
        print(scope_tab.to_string(index=False))
    ...
    if candidates.empty:
        print("[提示] 未生成任何候选变量对。")
        print("请检查：")
        print("1．metadata 中是否包含 dir_tag / dataset_class / survey_role / year；")
        print("2．分组后组内变量是否足够；")
        print("3．MIN_FINAL_SCORE 是否过高；")
        print("4．rough_similarity_filter 是否过严。")
        return
